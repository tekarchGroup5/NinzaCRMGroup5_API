...
public class api_BaseTest {
	...
	public void configSetup() throws IOException {
		...
		ExtentTest extentTest = extent.createTest(getClass().getSimpleName());// Create a new test report entry using
																				// the class name as the test name.
		...
		try (InputStream input = getClass().getClassLoader()// Reads the properties file from the classpath.
			...
			if (input == null)
				throw new RuntimeException("api_config.properties not found");// Stores key-value pairs from the config
																				// file (api_config.properties).
			prop.load(input);
		}
		...
		RequestSpecification requestSpecification = new RequestSpecBuilder()// All Request will be using these settings
				...
				// .addHeader("Accept", "application/json")//Adds Content-Type: application/json
				// header.
		...
		ResponseSpecification responseSpecification = new ResponseSpecBuilder()
				.expectHeader("Content-Type", "application/json").build();
//Sets these as the global default for all API requests and responses in tests.
		RestAssured.requestSpecification = requestSpecification;// Ensures all responses must have Content-Type:
																// application/json.
